    ...
    if (content && content.trim().length > 0) {
        if (typeof content === "string" &&
            ...
            try {
                ...
                if (parsed.type === "audio" && parsed.data && parsed.data.url) {
                    const audioElement = document.createElement("audio");
                    audioElement.controls = true;
                    audioElement.src = parsed.data.url;
                    audioElement.style.marginTop = "10px"; // Add margin to the top
                    messageDiv.appendChild(audioElement);

                    // for (const [key, value] of Object.entries(parsed.data.metadata)) {
                        //     const metadataItem = document.createElement("p");
                        //     metadataItem.textContent = `${convertToTitleCase(key)}: ${value}`;
                        //     metadataDiv.appendChild(metadataItem);
                        // }

                    if (parsed.data.metadata && parsed.data.metadata.reciter) {
                        const reciterElement = document.createElement("p");
                        reciterElement.classList.add("audio-reciter");
                        reciterElement.textContent = `Reciter: ${parsed.data.metadata.reciter}`;
                        messageDiv.appendChild(reciterElement);
                    }
                }
