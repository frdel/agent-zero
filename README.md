...
Agent Zero now supports selecting different models for chat and utility functionalities through dropdown menus in the UI. To configure available models:

1. **Edit `config.json`:** Ensure that the `available_models` section includes all desired models under the appropriate categories (`chat`, `utility`, `embedding`). Example:

    ```json
    {
        "chat_model": "claude-3.5-sonnet-20241022",
        "utility_model": "llama-3.2-90b-text-preview",
...
2. **Restart Agent Zero:** After updating `config.json`, restart the Agent Zero server to apply the changes.

3. **Select Models in UI:** Navigate to the chat or utility dropdowns in the UI to select your desired model from the available options.
