import models
from agent import AgentConfig, AgentAPIConfig
...
def initialize():
    ...
    # embedding model used for memory - shared across all agents for consistency
    ...
    # Define specialized configurations for different subordinate roles
    coder_config = AgentAPIConfig(
        chat_model=models.get_anthropic_chat(model_name="claude-3-opus-20240229", temperature=0),
        utility_model=models.get_openai_chat(model_name="gpt-3.5-turbo", temperature=0)
    )
    
    researcher_config = AgentAPIConfig(
        chat_model=models.get_openai_chat(model_name="gpt-4-turbo-preview", temperature=0),
        utility_model=models.get_openai_chat(model_name="gpt-3.5-turbo", temperature=0)
    )
    
    writer_config = AgentAPIConfig(
        chat_model=models.get_anthropic_chat(model_name="claude-3-sonnet-20240229", temperature=0.7),
        utility_model=models.get_openai_chat(model_name="gpt-3.5-turbo", temperature=0)
    )
    ...
    config = AgentConfig(
        ...
        # Role-specific API configurations for subordinate agents
        subordinate_configs={
            "coder": coder_config,
