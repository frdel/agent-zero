...
class Memory:  
    ...
    def initialize(  
        ...
        # Get the model name for namespace - handle LiteLLM models  
        model_name_for_namespace = getattr(embeddings_model, 'model_name', None) or getattr(embeddings_model, 'model', model_config.name)  
        embeddings_model_id = files.safe_file_name(  
            model_config.provider.name + "_" + model_name_for_namespace  
        )  
  
        # here we setup the embeddings model with the chosen cache storage  
        embedder = CacheBackedEmbeddings.from_bytes_store(  
            cast(Embeddings, embeddings_model), store, namespace=embeddings_model_id  
        )  
    ...
    def _cosine_normalizer(val: float) -> float:  
        ...
        res = max(  
            0, min(1, res)  
        )  # float precision can cause values like 1.0000000596  
        return res
