...
class DirtyJson:
    ...
    def _reset(self) -> None:
        ...
        self.result: Optional[JsonValue] = None
        self.stack: List[Union[Dict[str, JsonValue], List[JsonValue]]] = []

    @staticmethod
    def parse_string(json_string: str) -> JsonValue:
        parser = DirtyJson()
    ...
    def feed(self, chunk: str) -> JsonValue:
    ...
    def _continue_parsing(self) -> None:
    ...
    def _parse_object_content(self) -> None:
        while self.current_char is not None:
            self._skip_whitespace()
            if self.current_char == "}":
                if self._peek(1) == "}":  # Handle }}
    ...
    def _parse_array_content(self) -> None:
        while self.current_char is not None:
            self._skip_whitespace()
            if self.current_char == "]":
                self._advance()
