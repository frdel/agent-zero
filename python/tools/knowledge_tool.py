...
class Knowledge(Tool):
    ...
    async def perplexity_search(self, query):
        if dotenv.get_dotenv_value("API_KEY_PERPLEXITY"):
            return await asyncio.to_thread(
                perplexity_search.perplexity_search, query
            )
        else:
            PrintStyle.hint(
    ...
    async def duckduckgo_search(self, query):
        return await asyncio.to_thread(duckduckgo_search.search, query)

    async def searxng_search(self, query):
        return await searxng(query)

    async def mem_search(self, query: str):
        """Legacy memory search method for backward compatibility."""
        db = await memory.Memory.get(self.agent)
        docs = await db.search_similarity_threshold(
            query=query, limit=5, threshold=DEFAULT_MEMORY_THRESHOLD
        )
        text = memory.Memory.format_docs_plain(docs)
        return "\n\n".join(text)

    async def mem_search_enhanced(self, query: str):
        """
        ...
        try:
            ...
            if not all_docs:
                ...
                knowledge_docs = await db.search_similarity_threshold(
                    query=query, limit=5, threshold=lower_threshold,
                    filter="knowledge_source == True"
                )
                conversation_docs = await db.search_similarity_threshold(
                    query=query, limit=5, threshold=lower_threshold,
            ...
            if not all_docs:
                return await self._get_memory_diagnostics(db, query)
