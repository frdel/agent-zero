...
import torch  # Ensure PyTorch is installed and imported
...
class CodeExecution(Tool):
    ...
    def execute(self, **kwargs):
        if self.agent.handle_intervention(): return Response(message="",
                                                             break_loop=False)  # wait for intervention and handle it, if paused
    ...
    def after_execution(self, response, **kwargs):
        msg_response = files.read_file("./prompts/fw.tool_response.md", tool_name=self.name,
    ...
    def prepare_state(self):
        ...
        if not self.state:
            ...
            if self.agent.config.code_exec_docker_enabled:
                docker = DockerContainerManager(name=self.agent.config.code_exec_docker_name,
            ...
            if self.agent.config.code_exec_ssh_enabled:
                shell = SSHInteractiveSession(self.agent.config.code_exec_ssh_addr,
    ...
    def execute_python_code(self, code):
        # Determine if GPU is available; fallback to CPU if not
        device = "cuda" if torch.cuda.is_available() else "cpu"
        code = f"""
import torch
torch_device = torch.device('{device}')

# Insert the original user code below
{code}
"""
