...
- Fallback mechanism: If the primary model hits a rate limit, the system automatically falls back to a secondary model (default: GPT-4o-mini) to ensure continuous operation.
...
The project now includes a new directory structure to support the agent's functionality:

- **HDev/**: A new directory at the same level as 'agent-zero-main'
  - **work_dir/**: Contains subdirectories for various agent functions
    - **prompts_subdir/**: Directory for storing prompts
    - **memory_subdir/**: Directory for storing agent's memory
    - **knowledge_subdir/**: Directory for storing knowledge base

These new directories are created to give the agent access to dedicated spaces for prompts, memory, and knowledge. This structure allows for better organization and management of the agent's resources.
